...
export default function RundownDashboard() {
  ...
  const handleContentSubmit = async (e) => {
    ...
    setContents((prev) => [...prev, data].sort((a, b) => a.date.localeCompare(b.date)));
          ...
          <div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="lg:col-span-2 bg-white p-6 rd-line rounded">
                ...
                {contentsLoading ? (
                  <p className="text-xs text-gray-500 py-4 text-center">Chargement...</p>
                ) : contents.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {contents.slice(0, 3).map((item) => (
                      <div key={item.id} className="p-3 rd-line rounded flex items-center justify-between text-sm bg-gray-50">
                        ...
                        <span className="text-xs text-gray-500">{item.date} à {item.time ? item.time.slice(0, 5) : '00:00'}</span>
                      </div>
                    ))}
                  </div>
                )}
        ...
        return (
          <div>
            ...
            {contentsLoading ? (
              ...
              <p className="text-sm text-gray-500">Aucun contenu planifié pour le moment.</p>
            ) : (
                ...
                {contents.map((item) => (
                  <div key={item.id} className="bg-white p-4 rd-line rounded">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <span className="text-xs font-mono text-gray-500">{item.date} — {item.time ? item.time.slice(0, 5) : ''}</span>
                    </div>
                    <h4 className="font-semibold text-sm mb-2" style={{ color: '#1C2430' }}>{item.title}</h4>
                    {item.media_url && (
                      <p className="text-xs text-blue-600 truncate mb-2">📎 Média attaché : {item.media_url}</p>
                    )}
                  </div>
                ))}
              </div>
      ...
      case 'contenus':
        return (
          <div>
            ...
            {contentsError && (
              <div className="rd-mono text-xs mb-4 px-3 py-2 rounded" style={{ background: 'rgba(181,69,47,0.08)', color: '#B5452F' }}>
                {contentsError}
              </div>
            )}
                ...
                {contents.map((item) => (
                  <div key={item.id} className="bg-white p-4 rd-line rounded flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                      ...
                      {item.media_url && <p className="text-xs text-gray-500 mt-1">Média : {item.media_url}</p>}
                    </div>
                    <button
            ...
            <form onSubmit={handleContentSubmit}>
              ...
              <div className="flex justify-end gap-3">
                ...
                <button
                  ...
                  style={{ background: '#1C2430', color: '#F0F1EC', opacity: submitting ? 0.6 : 1 }}
                >
