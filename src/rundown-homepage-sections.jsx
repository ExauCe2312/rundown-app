...
const PLANS = [
  {
    ...
    features: [
      "1 plateforme suivie",
      "Calendrier de publication basique",
  ...
  },
  {
    ...
    features: [
      "Tout Créateur, plus :",
      "Comptes collaborateurs multiples",
      "Export de rapports",
      "Support prioritaire",
    ],
  },
...
const FOOTER_COLONNES = [
  {
    titre: "Produit",
    liens: ["Fonctionnalités", "Tarifs", "Nouveautés", "Feuille de route"],
  },
  {
    titre: "Ressources",
    liens: ["Centre d'aide", "Guides créateurs", "Blog", "Communauté"],
  },
  {
    titre: "Entreprise",
  ...
  return (
    <section style={{ background: "#F0F1EC" }}>
      <div className="max-w-5xl mx-auto px-6 py-20">
        <p
          className="rd-mono text-xs uppercase tracking-widest mb-3"
          style={{ color: "#5B6472" }}
        >
          Fonctionnalités
        </p>
        <h2
          className="rd-display text-3xl md:text-4xl font-semibold mb-3"
          style={{ color: "#1C2430" }}
        >
          Pensé pour le rythme d'un créateur
        </h2>
        <p className="text-base mb-12 max-w-xl" style={{ color: "#5B6472" }}>
          Chaque fonctionnalité existe pour une raison : vous faire gagner du
          temps, pas vous donner un écran de plus à surveiller.
        </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {FEATURES.map((f) => (
            <div key={f.n} className="flex gap-4">
              <span
                className="rd-mono text-sm shrink-0"
                style={{ color: "#B9B4A5" }}
              >
                {f.n}
              </span>
  ...
  return (
    <section style={{ background: "#FFFFFF" }}>
      <div className="max-w-5xl mx-auto px-6 py-20">
        ...
        <p className="text-base mb-12 max-w-xl" style={{ color: "#5B6472" }}>
          Changez de formule à tout moment. Aucune carte bancaire requise pour
          démarrer.
        </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-stretch">
          {PLANS.map((plan) => (
            <div
              ...
              style={
                plan.highlight
                  ? { background: "#1C2430", borderColor: "#1C2430" }
                  : { background: "#FFFFFF" }
              }
            >
              {plan.highlight && (
              ...
              <h3
                ...
                style={{ color: plan.highlight ? "#F0F1EC" : "#1C2430" }}
              >
                {plan.nom}
              </h3>
              <p
                className="text-sm mb-5"
                style={{ color: plan.highlight ? "#B7BEC8" : "#5B6472" }}
              >
                {plan.description}
              </p>
              <div
              ...
              >
                ...
                {plan.periode && (
                  <span className="text-sm ml-1">{plan.periode}</span>
                )}
              </div>
        ...
        <p className="rd-mono text-xs mt-8" style={{ color: "#9CA3AF" }}>
          * Tarifs indicatifs, à ajuster selon votre positionnement et vos coûts
          réels.
        </p>
      </div>
    </section>
      ...
      <div className="max-w-5xl mx-auto px-6 pt-16 pb-8">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-10 mb-12">
          <div className="md:col-span-2">
            <span
              className="rd-display text-xl font-semibold"
              style={{ color: "#F0F1EC" }}
            >
              Rundown
            </span>
            <p className="text-sm mt-3 max-w-xs" style={{ color: "#8891A0" }}>
              Le calendrier éditorial des créateurs qui n'ont pas de temps à
              perdre.
            </p>
          ...
          {FOOTER_COLONNES.map((col) => (
            <div key={col.titre}>
              <p
                className="rd-mono text-xs uppercase tracking-wide mb-3"
                style={{ color: "#8891A0" }}
              >
                {col.titre}
              </p>
        ...
        >
          <p className="rd-mono text-xs" style={{ color: "#6B7280" }}>
            © 2026 Rundown. Tous droits réservés.
          </p>
          <p className="rd-mono text-xs" style={{ color: "#6B7280" }}>
            Fait pour les créateurs, pas pour les tableurs.
          </p>
        </div>
      </div>
    </footer>
